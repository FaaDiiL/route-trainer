...
const CachedData = () => {
    ...
    const handleChange = (e) => {
        ...
        let filteredResult = result.filter(item => !(item.name === e.target.value))
...
const WeatherList = ({ data, handleChange }) => (
    <ul>
        {data.map((item, index) => (<WeatherItem key={index} item={item} handleChange={handleChange} />))}
    </ul>
);

const WeatherItem = ({ item,handleChange }) => (
    <li>{item.name} has {item.temp.toFixed(1)} degrees right now. {(item.wind.speed > 3) && `It's windy`} <button onClick={handleChange} value={item.name}>HIDE</button></li>
)
